...
///
/// Can be disabled with [`DISABLE_MOUSE_XY_TRACKING`].
///
...
/// The terminal will reply with `CSI M Cb Cx Cy` where:
/// - `Cb` is the button pressed on the mouse, event and modifiers. Value that
///   it represents is ordinary value of up to two byte utf-8 character - 32.
///   The value bits from lowest to highest (`76543210`) have this meaning:
///     - Bits `7610` form number representing the mouse button:
///         - `0` primary press (left)
///         - `1` middle press
///         - `2` secondary press (right)
...
///       pressed.
/// - `Cx` and `Cy` are character coordinates of the mouse press. They are
///   encoded as up to two byte utf-8 character where ordinary value of the
///   character - 32 is the value of the coordinate.
///
...
///
/// The limitation on the maximum value of `x` and `y` coordinate comes from
/// the fact that two byte utf-8 character character can have only value from 0
/// to 2047 and the first 32 characters are unused. (2047 - 32 == 2015)
///
/// # Example
/// ```no_run
/// use termal_core::{
...
/// }
///
/// print!("{}", codes::DISABLE_MOUSE_XY_UTF8_EXT);
/// print!("{}", codes::DISABLE_MOUSE_XY_ALL_TRACKING);
/// term.flush()?;
///
/// disable_raw_mode()?;
...
///
/// ## Result in terminal
/// ![](https://raw.githubusercontent.com/BonnyAD9/termal/refs/heads/master/assets/codes/enable_mouse_xy_utf8_ext.gif)
...
///
/// Equivalent to `OSC 1 1 ; Pc ST` where `Pc` is the color to set either as
/// `#RGB` or `rgb:R/G/B` where R, G and B are red green and blue components in
/// hex with 1 to 4 digits.
///
/// Can be reset with [`RESET_DEFAULT_BG_COLOR`].
///
/// # Example
/// ```no_run
/// use termal_core::codes;
...
///
/// Termal can parse the response as [`crate::raw::events::Event::Status`] with
/// [`crate::raw::events::Status::DefaultFgColor`]. So
/// the read event will match
/// `Event::Status(Status::DefaultFgColor(Rgb::<u16> { .. }))`.
///
/// # Example
/// ```no_run
/// use termal_core::{
...
///
/// Termal can parse the response as [`crate::raw::events::Event::Status`] with
/// [`crate::raw::events::Status::DefaultBgColor`]. So
/// the read event will match
...
///
/// Equivalent to `OSC ? 1 2 ST`.
///
/// The terminal will reply with `CSI 1 2 ; Pc ST` where `Pc` is the color
/// either in format `rgb:R/G/B` or `#RGB` where `R`, `G` and `B` is red, green
/// and blue component of color in hexadecimal with 1 to 4 digits.
///
/// Termal can parse the response as [`crate::raw::events::Event::Status`] with
/// [`crate::raw::events::Status::CursorColor`]. So the
/// read event will match
/// `Event::Status(Status::CursorColor(Rgb::<u16> { .. }))`.
///
/// # Example
/// ```no_run
/// use termal_core::{
...
///
/// ## Result in terminal
/// ![](https://raw.githubusercontent.com/BonnyAD9/termal/refs/heads/master/assets/codes/request_cursor_color.png)
...
///
/// ## Result in terminal
/// ![](https://raw.githubusercontent.com/BonnyAD9/termal/refs/heads/master/assets/codes/request_selection.png)
